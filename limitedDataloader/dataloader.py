...
def process_scene(
    ...
    for fl in channels:
        ...
        #    np.save(f"{temp_folder}/{i}_{fl}.npy", chips[fl][i])
        ...
        j = 0
        for i in range(len(chips[fl])):
            if i in pixel_detections["chip_index"]:
                np.save(f"{temp_folder}/{j}_{fl}.npy", chips[fl][i])
                j += 1

    # Print number of detections per scene; make sure it aligns with
    # number expected
    chip_indices_len = len(chip_indices)
    if detections is not None:
...
class XView3Dataset(object):
	...
	if detect_file and self.detections is not None:
    ...
    def __getitem__(self, idx):
        ...
        for fl in self.channels:
            ...
            if self.min_max_norm:
                # Puts values b/t 0 and 1, as expected by Faster-RCNN 
    ...
    def add_background_chips(self):
        ...
        for scene_id in self.scenes:
            ...
            num_background = 2
            # = int(self.background_frac * max(len(scene_detect_chips),self.background_min))
