...
def process_scene(
    ...
    if detections is not None:
        ...
        #print(f"Detections expected for scene # {index} ({scene_id}): {scene_detect_num}")
    ...
    if detections is not None:
        ...
        #print(f"Detections recovered in chips for scene {scene_id}: {chip_detect_num} \n")
...
class XView3Dataset(object):
    ...
    def __getitem__(self, idx):
        ...
        for fl in self.channels:
            ...
            # data[fl] = (data[fl] - m) / (M - m) if np.abs(M-m) != 0 else 0
                if np.abs(np.min(data[fl]) - np.min(data[fl])) > 0:
                    data[fl] = (data[fl] - np.min(data[fl])) / (np.max(data[fl]) - np.min(data[fl]))
                else:  # zero-ed chip
                    data[fl]  = np.zeros(data[fl].shape, dtype=data[fl].dtype)
        ...
        if self.detections is not None:
            ...
            if (num_objs == 1) and (detects.iloc[0].vessel_class == BACKGROUND):
                ...
                for i in range(num_objs):
                    ...
                    bbox_length = self.bbox_size
                    xmin = detect.columns - bbox_length
                    xmax = detect.columns + bbox_length
                    ymin = detect.rows - bbox_length
                    ymax = detect.rows + bbox_length
...
def generate_loaders(train_data, val_data, bs):
    train_sampler = torch.utils.data.RandomSampler(train_data)
    val_sampler = torch.utils.data.SequentialSampler(val_data)
    
    data_loader_train = torch.utils.data.DataLoader(
    train_data, batch_size=bs, sampler=train_sampler, num_workers=0,
...
def generate_loaders_unlabeled(val_data_root, val_data, chips_path):
    ...
    data_loader_val_unlabeled = torch.utils.data.DataLoader(
            ...
            pin_memory=True)
    return data_loader_val_unlabeled