...
def predict_results(data_loader_val_unlabeled, model_eval, device):
    res = {}

    with torch.no_grad():
        for image, targets in tqdm(data_loader_val_unlabeled, total=len(data_loader_val_unlabeled)):
            image = list(img.to(device) for img in image)
            targets = [{k: (v.to(device) if not isinstance(v, str) else v)
                        for k, v in t.items()}
                       for t in targets]
            ...
            with torch.no_grad(): outputs = model_eval(image)
            outputs = [{k: v.to("cpu") for k, v in t.items()} for t in outputs]

            # Translating from bounding boxes to point predictions
            for jj, (target, output) in enumerate(zip(targets, outputs)):
                output.update({"pred_chip_columns": [[int(np.mean([box[0], box[2]])) for box in output["boxes"]]
                                                     if len(output["boxes"]) > 0
                                                     else []]})
                output.update({"pred_chip_rows": [[int(np.mean([box[1], box[3]])) for box in output["boxes"]]
                                                  if len(output["boxes"]) > 0
                                                  else []]})

                # Estimating length using the size of the bounding box            
