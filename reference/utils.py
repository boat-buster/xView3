...
from shutil import copytree as copyd
...
def format_data(data_path):
    ...
    for dr in os.listdir(data_path):
        if dr.endswith('t'): os.rename(f'{data_path}/{dr}', f'{data_path}/train/{dr}')
        elif dr.endswith('v'): os.rename(f'{data_path}/{dr}', f'{data_path}/validation/{dr}')
...
def new_model(chips_path, overwrite_preproc, train_data):
    ...
    model = xView3BaselineModel(num_classes=num_classes, num_channels=len(train_data.channels), image_mean=image_mean, image_std=image_std)
    return model

def save_checkpoint(model, optimizer, lr_scheduler, epoch):
    model_path = f'trained_model_{epoch+1}_epochs.pth'
    checkpoint_path = f'trained_checkpoint_{epoch+1}_epochs.pth'
    
    # Save checkpoint - both model only and checkpoint object
    torch.save(model.state_dict(), model_path)
    torch.save({'epoch': epoch, 'model': model.state_dict(), 'optimizer': optimizer.state_dict(), 'scheduler': lr_scheduler}, checkpoint_path)
    return

def display(df_out):
    print(f"{len(df_out)} detections found")
    df_out.to_csv('/data/xview3/data/inference_output.csv', index=False)
    print(df_out)
    
def read_in_dets(val_label_file):
    inference = pd.read_csv('/data/xview3/data/inference_output.csv', index_col=False)
    ...
    return inference, ground_truth
    
def record_score(out):
    params = open('results.txt', 'w')
    params.write("Results:\n")
    params.write(str(out))
    params.close()
    print(out)
    return
    
def record_params(bs, lr, moment, decay, num_epochs):
    params = open('parameters.txt', 'w')
    params.write("Parameters:\n")
    params.write(f" learning rate: {lr}\n")
    params.write(f" momentum: {moment}\n")
    params.write(f" weight decay: {decay}\n")
    params.write(f" batch size: {bs}\n")
    params.write(f" epochs: {num_epochs}\n")
    params.close()
    print(f"Lr: {lr}, Momentum: {moment}, Weight Decay: {decay}, Batch Size: {bs}, Epochs: {num_epochs}")
    return
    
def record_all(epochs):
    today = date.today().strftime("%d%B")
    copyd('/data/xview3/data/big/visuals', f'/data/xview3/data/results/results{today}')
    copyf('parameters.txt', f'/data/xview3/data/results/results{today}')
    copyf('results.txt', f'/data/xview3/data/results/results{today}')
    copyf(f'trained_model_{epochs}_epochs.pth', f'/data/xview3/data/results/results{today}')
    return
...
def reformat_results(res, chips_path):
    # Instantiate output dataframe in xView3-specified format
    df_out = output_dataframe()
    
    # Formatting output in xView3 format; converting chip-level detection coordinates to scene-level detection coordinates
    for inf_img_id, inf_val in tqdm(res.items()):
        for idx, box in enumerate(inf_val["boxes"]):
            ...
            with open(f"{chips_path}/validation/{scene_id}/coords.json") as fl: coords = json.load(fl)
            ...
            df_out.loc[len(df_out)] = [scene_pred_row, scene_pred_column,scene_id, is_vessel, is_fishing, length]
